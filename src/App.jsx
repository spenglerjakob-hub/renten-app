...
export default function App() {
  ...
  const [grvIncreaseRate, setGrvIncreaseRate] = useState(1.0);
  ...
  const [printExplanationMode, setPrintExplanationMode] = useState('detailed');
  ...
  const [planerReturn, setPlanerReturn] = useState(2.0);
  ...
  const handleAddTuevItemSelect = (e) => {
      addTuevItem(e.target.value);
  ...
  const calculations = useMemo(() => {
    ...
    const processedContracts = contracts.map(c => {
      ...
      else if (c.type === 'bav' || c.type === 'bavUkasse') {
        if (c.type === 'bavUkasse') {
             const ukVfb = getVersorgungsfreibetrag(cRetYear, c.gross * 12);
             zvE_contribution = Math.max(0, c.gross - ukVfb.actualFreibetragMonthly - (102/12));
        } else {
    ...
    const finalizedContracts = processedContracts.map(c => {
      ...
      if (['basis', 'bav', 'bavUkasse', 'riester', 'prvRente'].includes(c.type)) {
      ...
      if (c.compareMode && Number(c.compareRenteGross) > 0 && Number(c.compareCapitalGross) > 0) {
          const rGross = Number(c.compareRenteGross);
          const cGross = Number(c.compareCapitalGross);
          let nRente = 0, nCap = 0;
          const cErtRate = c.owner === 'B' && isMarried ? ertragsanteilRateB : ertragsanteilRateA;
          const cRetAge = c.owner === 'B' && isMarried ? retirementAgeB : retirementAgeA;
          const simRemainingBBG = isMarried ? BBG_KV * 2 : BBG_KV;

          let rTax = 0, rKist = 0, rKvpv = 0, rZve = 0;
          if (c.type === 'bav' || c.type === 'bavKapital' || c.type === 'bavUkasse') {
              if (c.type === 'bavUkasse') {
                  const ukVfbComp = getVersorgungsfreibetrag(c.cRetYear, rGross * 12);
                  rZve = Math.max(0, rGross - ukVfbComp.actualFreibetragMonthly - (102/12));
              } else {
                  rZve = c.isOldContract ? rGross * cErtRate : rGross;
              }
              if (kvStatus === 'kvdr') {
                  rKvpv = Math.max(0, rGross - bavFreibetragKV) * kvRateFull + (rGross > bavFreibetragKV ? rGross * pvRateFull : 0);
              } else if (kvStatus === 'freiwillig') {
          ...
          if (c.type === 'bav' || c.type === 'bavKapital' || c.type === 'bavUkasse') {
              ...
              if (kvStatus === 'freiwillig') {
                  cKvpv = Math.min(estimatedProfit / 120, Math.max(0, simRemainingBBG)) * (kvRateFull + pvRateFull) * 120;
              }
              if (!c.isOldContract) {
                  const taxHalb = Math.max(0, (calculateESt(zvE_yearly_today + (estimatedProfit / taxInflationFactor) * 0.5 * 0.85, isMarried) - tax_today) * taxInflationFactor);
                  const taxAbgeltung = estimatedProfit * 0.85 * (hasChurchTax ? 0.278186 : 0.26375);
                  if ((taxHalb + taxHalb * kistRate) < taxAbgeltung) { cTax = taxHalb; cKist = taxHalb * kistRate; } else { cTax = taxAbgeltung; cKist = 0; }
              }
          }
          nCap = Math.max(0, cGross - cTax - cKist - cKvpv);

          const breakEvenAge0 = nRente > 0 ? cRetAge + (nCap / (nRente * 12)) : cRetAge;
          let breakEvenAge2 = 0, isPerpetual = false;
          const r_m = 0.02 / 12;
          if (nRente > 0) {
              if (nRente <= nCap * r_m) {
                  isPerpetual = true;
              } else {
                  const months = -Math.log(1 - (nCap * r_m) / nRente) / Math.log(1 + r_m);
                  breakEvenAge2 = cRetAge + (months / 12);
              }
          }

          compareResult = { netRente: nRente, netCapital: nCap, breakEvenAge0, breakEvenAge2, isPerpetual };
      }
  ...
  const renderContractInput = (c) => {
    ...
    const typeLabels = {
      'basis': 'Rürup / Basis', 'bav': 'bAV (Rente)', 'bavUkasse': 'Unterstützungskasse', 'bavKapital': 'bAV (Kapital)', 'riester': 'Riester',
    ...
    <div key={c.id} className="bg-white border border-slate-200 rounded-lg shadow-sm mb-2 sm:mb-3 print:border-slate-300 print:shadow-none overflow-hidden">
      ...
      <div className={`${isExpanded ? 'block' : 'hidden'} print:block p-3 sm:p-4 relative`}>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
          <div>
            ...
            <select value={c.type} onChange={e => handleContractTypeChange(c.id, e.target.value)} className="w-full border border-slate-300 rounded p-2 text-xs sm:text-sm bg-slate-50">
              ...
              {c.layer === 2 && <><option value="bav">bAV (Rente)</option><option value="bavUkasse">bAV (Unterstützungskasse §19)</option><option value="bavKapital">bAV (Kapital)</option><option value="riester">Riester-Rente</option></>}
        ...
        {(c.type === 'bav' || c.type === 'bavKapital' || c.type === 'bavUkasse' || c.type === 'prvRente' || c.type === 'prvKapital') && (
          <div className={`${(c.type === 'bavUkasse') ? 'mt-3 pt-3 border-t border-slate-100' : ''}`}>
  ...
  const renderBonContract = (c) => {
    ...
    if (isKapital) {
       ...
       if (c.type === 'bavUkasse') {
           subtitle = `Brutto: ${formatResultCurrency(Number(c.gross) || 0)} | §19 Versorgungsbezug (Freibetrag)`;
       } else {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans pb-40 sm:pb-36 print:bg-slate-50 print:pb-0" style={{ WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}>
            ...
            {showBasisdaten && (
              <div className="p-3 sm:p-5 pt-4">
                ...
                <div className="mb-4 p-3 sm:p-4 bg-indigo-50/70 rounded-xl border-2 border-indigo-100 shadow-sm print:bg-indigo-50 print:border-indigo-200">
                  ...
                  <input type="number" value={targetIncomeToday} onChange={e => setTargetIncomeToday(parseNum(e.target.value))} className="w-full border-2 border-indigo-200 bg-white rounded-lg p-2.5 sm:p-3 text-lg sm:text-xl font-black text-indigo-900 mb-2 shadow-inner outline-none focus:border-indigo-400 transition-colors" />
      ...
      <div className="max-w-6xl mx-auto p-2 sm:p-6 mb-24 print:block print:break-before-page">
         <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4 sm:mb-6 border-b-2 border-amber-200 pb-3 sm:pb-4 px-2 sm:px-0">
             ...
             <div className="shrink-0 bg-amber-100 p-1.5 sm:p-2 rounded-xl border border-amber-200 shadow-sm w-full md:w-auto relative z-10">
                 <select value="" onChange={handleAddTuevItemSelect} className="bg-white border-2 border-amber-300 text-amber-900 rounded-lg p-2 sm:p-2.5 text-xs sm:text-sm font-bold shadow-sm outline-none hover:border-amber-400 cursor-pointer w-full">
                     <option value="">➕ Vertrag hinzufügen...</option>
                     {contracts.filter(c => ['basis', 'bav', 'bavUkasse', 'bavKapital', 'riester', 'prvRente', 'prvKapital', 'etf'].includes(c.type)).map(c => (
                         ...
                         <div key={item.id} className="bg-white rounded-xl shadow-md border border-slate-200 shrink-0 flex flex-col lg:flex-row print:flex-row overflow-hidden relative w-full print:break-inside-avoid print:shadow-none print:border-slate-300">
                             ...
                             <div className={`w-full lg:w-[30%] print:w-[30%] p-3 sm:p-5 print:p-4 bg-slate-50 border-b lg:border-b-0 print:border-b-0 lg:border-r print:border-r border-slate-200 ${item.payoutGross === 0 ? 'pt-10 sm:pt-12 print:pt-10' : ''}`}>
                                 <div className="flex justify-between items-start mb-3 sm:mb-5 print:mb-3">
                                     <div>
                                         <div className="text-[9px] sm:text-[11px] print:text-[11px] font-bold uppercase tracking-wider text-slate-500 mb-0.5 sm:mb-1">Schicht {item.layer} | {item.cType.replace('bavUkasse', 'u-kasse').toUpperCase()}</div>
                             ...
                             {/* RIGHT COLUMN: KPIs & Fazit */}
                             <div className="w-full lg:w-[38%] print:w-[38%] bg-indigo-900 text-white p-3 sm:p-5 print:p-4 flex flex-col justify-center">
                                 <div className="text-[10px] sm:text-[11px] font-bold text-indigo-300 uppercase tracking-wider mb-3">Auswertung & Rendite</div>
      ...
      {/* FIXED BOTTOM BAR */}
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-slate-900 border-t border-slate-800 shadow-[0_-10px_20px_-5px_rgba(0,0,0,0.3)] print:hidden transition-transform duration-300">
        <div className="max-w-6xl mx-auto px-2 sm:px-4 py-2.5 sm:py-3">
          <div className="flex justify-between items-center gap-1.5 sm:gap-4">
            {/* Zielbedarf */}
            <div className="flex-1 bg-slate-800 p-1.5 sm:p-2.5 rounded-lg border border-slate-700 text-center">
              <div className="text-[10px] sm:text-xs text-slate-400 font-bold uppercase tracking-wider mb-0.5 sm:mb-1">Zielbedarf</div>
              <div className="text-base sm:text-xl font-bold text-white">{formatResultCurrency(calculations.targetIncomeFuture)}</div>
            </div>
            
            <div className="text-slate-500 font-black text-[10px] sm:text-sm">-</div>
            
            {/* Gesamt-Netto */}
            <div className="flex-1 bg-slate-800 p-1.5 sm:p-2.5 rounded-lg border border-slate-700 text-center">
              <div className="text-[10px] sm:text-xs text-slate-400 font-bold uppercase tracking-wider mb-0.5 sm:mb-1">Gesamt-Netto</div>
              <div className="text-base sm:text-xl font-bold text-indigo-400">{formatResultCurrency(calculations.totalNetFuture)}</div>
            </div>
            
            <div className="text-slate-500 font-black text-[11px] sm:text-sm">=</div>
            
            {/* Lücke */}
            <div className={`flex-1 p-1.5 sm:p-2.5 rounded-lg border text-center shadow-inner ${calculations.gap > 0 ? 'bg-rose-900/30 border-rose-700/50 text-rose-400' : 'bg-emerald-900/30 border-emerald-700/50 text-emerald-400'}`}>
              <div className={`text-[10px] sm:text-xs font-bold uppercase tracking-wider mb-0.5 sm:mb-1 ${calculations.gap > 0 ? 'text-rose-500' : 'text-emerald-500'}`}>Versorgungslücke</div>
              <div className="text-base sm:text-xl font-black">{calculations.gap > 0 ? formatResultCurrency(calculations.gap) : 'Gedeckt'}</div>
            </div>
          </div>
        </div>
      </div>
